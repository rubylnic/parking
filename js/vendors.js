...
if (localMap) {
  let cooords = localMap.dataset.coords;
  let solitted = cooords.split(',').map(function(cooord) {
    return parseFloat(cooord)
  });
  var coordsNew = {
    coords: solitted,
  };
...
class YmapsInitializerFew {
  constructor(container, placeCoords = coordsNew) {
  ...
  addMapDetails(mapCoords) {
    const init = () => {
      ...
      this.myPlacemark = new ymaps.Placemark(this.myMap.getCenter(), {
        iconContent: localMap.dataset.number,
        hintContent: document.querySelector('.local-map__address').textContent
      }, {
...
const coords = {
  coords: [59.899884, 30.501568],
  name: 'Автостоянка 24',
  description: "Автостоянка в Санкт-Петербурге",
};
const parkings = document.querySelectorAll('.parking-info');
...
class YmapsInitializer {
  constructor(container, placeCoords = coords) {
    ...
    this.container = container;
    this.coords = placeCoords;
    this.url = 'https://api-maps.yandex.ru/2.1/?lang=ru_RU&amp;apikey=71255b8e-f2f1-47a3-8195-b51ba2bb1327>';
    this.myMap = null;
    this.mapContainer = null;
    this.apiScript = null;
    this.myPlacemark = null;
  ...
  addMapDetails(mapCoords) {
    const init = () => {
      ...
      this.parkings.forEach((card, index) => {
          console.log(card.querySelector('.parking-info__coords'));
          let coords = card.querySelector('.parking-info__coords').textContent.split(',').map(function(cooord) {
            return parseFloat(cooord)
          });
          console.log(coords);

          this.myMap.geoObjects.add(new ymaps.Placemark(coords, {
            iconContent: 'P' + (index + 1),
            hintContent: card.querySelector('.parking-info__name').textContent,
            balloonContentHeader: '<div class="map__title" href = "#">' + card.querySelector('.parking-info__name').textContent + '</div>',
            // Зададим содержимое основной части балуна.
            balloonContentBody: '<div class="map__places"><div class="map__place "><span class="free">' + card.querySelector('.parking-info__free').textContent + '</span> свободно</div> <div class="map__place "><span class="taken">' + card.querySelector('.parking-info__taken').textContent + '</span> арендовано</div></div>' +
              '<div class="map__price">Стоимость паркинга - <span>' + card.querySelector('.parking-info__price').textContent + '</span></div>',
            // Зададим содержимое нижней части балуна.
            balloonContentFooter: '<a class="map__link" href="' + card.querySelector('.parking-info__link').textContent + '">Перейти</a>',
